...
public class RangeManager : MonoBehaviour
{
    ...
    /// <returns></returns>
    public List<Square> CalculateRange(Square departure, int minRange, int maxRange)
    {
        List<Square> squaresNotInRange = new();
        List<Square> squaresInRange = new();
        List<Square> squaresOpen = new();

        squaresNotInRange.Add(departure);
        ...
        for (int i = 1; i < minRange; i++)
        {
            ...
            for (int j = 0; j < squaresToCheck.Count; j++)
            {
                ...
                for (int k = 0; k < currentSquaresToCheck.Neighbors.Count; k++)
                {
                    ...
                    if (currentNeighbor != null && !squaresNotInRange.Contains(currentNeighbor) && currentNeighbor)
                    {
